...
export const get_appointments = async (
  ...
  return mockAppointments.filter((app) => {
    ...
    if (filters.doctorName && app.doctorName !== filters.doctorName)
      matches = false;
    return matches;
  });
};
...
// 3. Mutation Function
export const update_appointment_status = async (
  id: string,
...
export const create_appointment = async (
  ...
  if (hasConflict) {
    throw new Error(
      `Time conflict detected with existing appointment for ${payload.doctorName}`
    );
  }
